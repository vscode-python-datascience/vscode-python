...
export class DataScienceCodeLensProvider implements IDataScienceCodeLensProvider {
    ...
    public getCodeWatcher(document: vscode.TextDocument): ICodeWatcher {
        return this.matchWatcher(document.fileName, document.version);
    }

    private matchWatcher(fileName: string, version: number) : ICodeWatcher {
        ...
        if (index >= 0) {
            ...
            // If we have an old version remove it fromthe active list
