...
import { URL } from 'url';
import { ExecutionResult, IPythonExecutionService, ObservableExecutionResult, Output, IPythonExecutionFactory } from '../common/process/types';
...
export class JupyterProcess implements INotebookProcess {
    ...
    public spawn = async (notebookFile: string) : Promise<ExecutionResult<string>> => {
        ...
        // Use the IPythonExecutionService to find Jupyter
        const pythonService = await this.executionFactory.create({});
        return await pythonService.execModule('jupyter', args, {throwOnStdErr: true, encoding: 'utf8'});
    }
